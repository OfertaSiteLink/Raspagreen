<?php
 ...
 if (!file_exists($autoload)) {
     ...
     echo json_encode([
         ...
         'details' => 'Execute "composer install" ou "composer require mercadopago/dx-php" na raiz do projeto para instalar dependências.'
     ]);
     exit;
 }
 ...
 // Roteamento inicial e health antes de exigir token
 $action = isset($_GET['action']) ? $_GET['action'] : null;
 if ($action === 'health') {
     header('Content-Type: application/json');
     $tokenEnv = getenv('MP_ACCESS_TOKEN');
     if (!$tokenEnv && isset($_ENV['MP_ACCESS_TOKEN'])) {
         $tokenEnv = $_ENV['MP_ACCESS_TOKEN'];
     }
     echo json_encode([
         'status' => 'ok',
         'php_version' => PHP_VERSION,
         'sapi' => php_sapi_name(),
         'composer_autoload' => file_exists($autoload),
         'token_present' => (bool)$tokenEnv,
         'time' => date('c')
     ]);
     exit;
 }

 // Configurar credenciais via variável de ambiente (mais seguro)
 $mpToken = getenv('MP_ACCESS_TOKEN');
 if (!$mpToken && isset($_ENV['MP_ACCESS_TOKEN'])) {
 ...
 if (!$mpToken) {
     ...
     echo json_encode([
         ...
         'details' => 'Defina a variável de ambiente MP_ACCESS_TOKEN no servidor antes de usar a API.'
     ]);
     exit;
 }
